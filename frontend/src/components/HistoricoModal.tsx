...
import { X, Save, BookOpen, Hash } from "lucide-react";
import type { Historico } from "../types";

interface HistoricoModalProps {
    isOpen: boolean;
    onClose: () => void;
    onSave: (data: Partial<Historico>) => Promise<void>;
...
export default function HistoricoModal({
    ...
    useEffect(() => {
        if (isOpen) {
            if (historico) {
                setCodigo(historico.codigo);
                setDescricao(historico.descricao);
                setAtivo(historico.ativo);
            } else {
                resetForm();
            }
            setErrors({});
            ...
            >
                ...
                <div className="flex items-center justify-between p-6 border-b border-slate-700/50">
                    <h2 className="text-2xl font-bold text-white flex items-center gap-3">
                        ...
                        {historico ? "Editar Histórico" : "Novo Histórico"}
                    </h2>
                    <button
                        onClick={onClose}
                ...
                <form onSubmit={handleSubmit} className="p-6">
                    <div className="space-y-6">
                        ...
                        <div>
                            ...
                            <div className="relative">
                                <BookOpen className="absolute left-4 top-1/2 transform -translate-y-1/2 text-slate-400" size={20} />
                                <input
                                    type="text"
                                    value={descricao}
                ...
                <div className="flex items-center justify-end gap-3 p-6 border-t border-slate-700/50">
                    ...
                    >
                        <Save size={20} />
                        {isSubmitting ? "Salvando..." : historico ? "Atualizar" : "Salvar"}
                    </button>
                </div>
            </div>
        </div>
